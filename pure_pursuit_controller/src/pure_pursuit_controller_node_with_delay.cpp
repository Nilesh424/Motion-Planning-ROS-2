/* 
@file: pure_pursuit_controller_node_with_delay.cpp
@description: Pure Pursuit Controller Node with delay converted to ROS 2. Subscribes to the path planner topic loaded in the parameter server to get waypoints.
*/
...
class PurePursuitControllerWithDelay : public rclcpp::Node
{
private:
    ...
    int path_seq_;
    int path_size_;
    int current_best_waypoint_seq_;
    float max_steering_angle_ = 0.4189;
    float mid_speed_ = 3.0;
    float slowest_speed_ = 2.0;
    float max_speed_ = 5.0; // Speed 5 is optimum.
    bool path_detected_;
...
public:
    // Constructor
    PurePursuitControllerWithDelay();
...
PurePursuitControllerWithDelay::PurePursuitControllerWithDelay()
    : Node("pure_pursuit_controller_node_with_delay")
{
    // Declare parameters with default values
    this->declare_parameter<std::string>("planner_topic", "/planned_path");
    this->declare_parameter<float>("lookahead_distance_", 0.9);
    this->declare_parameter<float>("goal_threshold_distance_", 0.5);
    this->declare_parameter<int>("iterations_to_delay_", 40);
    ...
    iterations_to_delay_ = this->get_parameter("iterations_to_delay_").as_int();

    // Initialize subscribers and publishers
    path_subscriber_ = this->create_subscription<nav_msgs::msg::Path>(
        planner_topic_, 1, std::bind(&PurePursuitControllerWithDelay::path_subscriber_callback, this, std::placeholders::_1));
    car_pose_subscriber_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        "/gt_pose", 100, std::bind(&PurePursuitControllerWithDelay::car_pose_callback, this, std::placeholders::_1));
    ...
    path_seq_ = 0;
    current_best_waypoint_seq_ = 0;
    path_detected_ = false;
    iterations_of_delay_ = iterations_to_delay_;
    prev_steering_angle_ = 0.0;
    prev_velocity_ = 0.0;

    RCLCPP_INFO(this->get_logger(), "Pure Pursuit Controller with delay node initialized");
}

// Helper Functions Implementations
int PurePursuitControllerWithDelay::goal_closeness(const std::pair<float, float>& car_position)
{
...
float PurePursuitControllerWithDelay::dist(const geometry_msgs::msg::PoseStamped& pose1, const geometry_msgs::msg::PoseStamped& pose2)
{
...
void PurePursuitControllerWithDelay::stop_car()
{
...
void PurePursuitControllerWithDelay::path_subscriber_callback(const nav_msgs::msg::Path::SharedPtr planner_path)
{
...
void PurePursuitControllerWithDelay::car_pose_callback(const geometry_msgs::msg::PoseStamped::SharedPtr car_pose)
{
    // Pure pursuit algorithm implemented here.
    if (path_detected_ && (iterations_of_delay_ == iterations_to_delay_))
    {
    ...
    else if (path_detected_ && (iterations_of_delay_ != iterations_to_delay_))
    {
        ackermann_msgs::msg::AckermannDriveStamped drive_command;
        drive_command.header.stamp = this->now();
        drive_command.header.frame_id = "base_link";
...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<PurePursuitControllerWithDelay>();
    RCLCPP_INFO(node->get_logger(), "Pure Pursuit Controller with delay node has been created!");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
