/* 
@file: pure_pursuit_controller_node_with_lingering_lookahead_point.cpp
@description: Pure Pursuit Controller Node with lingering lookahead point converted to ROS 2.
Subscribes to the path planner topic loaded in the parameter server to get waypoints.
*/
...
private:
    ...
    geometry_msgs::msg::PointStamped lingering_lookahead_point_map_frame_;
    ...
    int iterations_to_wait_;
    int iterations_waited_;
...
PurePursuitController::PurePursuitController()
    : Node("pure_pursuit_controller_node_with_lingering_lookahead_point")
{
    ...
    this->declare_parameter<int>("iterations_to_wait", 5);
    ...
    iterations_to_wait_ = this->get_parameter("iterations_to_wait").as_int();
    ...
    car_pose_subscriber_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
        "/gt_pose", 100, std::bind(&PurePursuitController::car_pose_callback, this, std::placeholders::_1));
    ...
    iterations_waited_ = iterations_to_wait_;

    RCLCPP_INFO(this->get_logger(), "Pure Pursuit Controller node with lingering lookahead point initialized");
}
...
void PurePursuitController::car_pose_callback(const geometry_msgs::msg::PoseStamped::SharedPtr car_pose)
{
    ...
    if (path_detected_)
    {
        ...
        // Using the old lookahead point to calculate steering angle for "iterations_to_wait_" number of times.
        if (iterations_waited_ < iterations_to_wait_)
        {
            RCLCPP_INFO(this->get_logger(), "Using old lookahead point. Iterations waited: %d", iterations_waited_);
            geometry_msgs::msg::PointStamped target_point_car_frame;
            tf2::doTransform(lingering_lookahead_point_map_frame_, target_point_car_frame, map_to_car_frame);
        ...
        else
        {
            ...
            for (int i = path_seq_; i < (path_size_ - 1); i++)
            {
                ...
                else
                {
                    ...
                    if (t1 >= 0 && t1 <= 1)
                    {
                        ...
                        target_point_map_frame.point.z = 0.0;
                        lingering_lookahead_point_map_frame_ = target_point_map_frame;
            ...
            if (intersection_found)
            {
                ...
                target_point_y = target_point_car_frame.point.y;
                target_point_dist = sqrt(target_point_x * target_point_x + target_point_y * target_point_y);
                iterations_waited_ = 0;
            }
            else
            {
                RCLCPP_INFO(this->get_logger(), "No intersection between lookahead radius circle and path found.");
                // Since no point of intersection found, find the closest point on the path and track that.
        ...
        {
            stop_car();
            // Once goal reached, reset class members so that next path can be followed correctly.
            path_seq_ = 0;
            path_detected_ = false;
            target_point_x = 0.0;
            target_point_y = 0.0;
...
int main(int argc, char** argv)
{
    ...
    RCLCPP_INFO(node->get_logger(), "Pure Pursuit Controller node with lingering lookahead point has been created!");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
